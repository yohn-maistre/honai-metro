...
- Static data + honest live layers. `wiki/index.ts:4` eager-globs `*.md`; `data/*.json` holds `directory.json` (with optional per-community `region`), `orgs.json` (8 Level-0 contoh, `jenis` + structured `jam`), `bahasa.json`, `registry.json`, `agents.json`, and dashboard snapshots (all `demo:true`). `wiki/wajah.json` = 13 live-verified Wikipedia entries (see CLAUDE.md rule). Routes `load()` these and render with pure-SVG charts (`charts/`).
- Canvas atlas stack (no MapLibre anywhere): `atlas.ts` (dot grid + `loadBoundaryPaths` Path2D), `layers.ts` (live fetchers: BMKG/USGS gempa, Open-Meteo, PetaBencana, detak worker `/geo/*`; SWR localStorage, Papua clip, null=segera / []=nihil / points=langsung), `PetaKabar.svelte` (home hero, 6 toggle layers), `PetaSimpul.svelte` (/explore nodes), `WajahTanah.svelte` (wiki feature + mini locator).
- Live-ish helpers: `live.ts` (forum stats), `hot.ts` (single-flight Hot/Local), `kabar.ts` (kliping pins), `kilas-data.ts` (ticker), `musrenbang.ts` (`MUSRENBANG_COMMUNITY` + `fetchUsulan`), `org.ts` (`bukaSekarang` WIT), `seed.ts` (day clock), `capabilities.ts`, `tts.ts`.
- (2026-07-11: `PapuaMap.svelte`/`PetaPapua.svelte` long deleted; `WikiCarousel.svelte` retired for `WajahTanah` + `SejarahHariIni` + `KataHariIni`; `sorotan.json` deleted earlier.)
