...
- **Hermes Agent commitment** — *resolved 2026-06-08.* The Master Proposal v3.5+ explicitly commits to **Hermes Agent (Nous Research, open-source runtime — MIT, distinct from the Hermes-3 model series)** as the reference and primary planner-executor runtime. The DID + MCP + UCAN capability ceremony documented above remains harness-agnostic — any conforming runtime can implement it — but Hermes Agent is the supported and expected runtime for the Paket Kelurahan MVP and grant year. Alternative harnesses (Pi, vanilla Anthropic SDK harness, custom) are not blocked but are not first-class. See `docs/research/hermes-agent-deep-dive.md` for the deep-dive that informed this commitment, including the nine-piece harness anatomy, the tool registration ⊥ exposure split that enforces our reasoning⊥execution rule, and the persona-as-skill-registries (not separate profiles) model.
